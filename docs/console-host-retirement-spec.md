...
The unit may keep the name `gitboard.service` for one compatibility step, but the executable path must not reference `apps/gitboard`.
...
- Keep unit name `gitboard.service` for one cutover step or introduce `console.service` immediately?
- Should `/gitboard` redirect permanently to `/console`, or serve a temporary deprecation page?
- Which internal verify routes are still required after the host moves?
