...
| `source-lifecycle` | Core contracts defined as of `forge-6oae.6`; app implementations still own scanner/watcher runtime | `@xtrm/core/runtime` and `@xtrm/core/state` | `apps/gitboard` still owns `ProjectScanner`, `UnifiedScanner`, source routes, and watchers until CRITICAL `makeSourceHealth`/scanner consumers are migrated behind parity gates | Core source-health/source-lifecycle contract tests and existing app source/API tests passed |
