...
import { resolveBeadsProjectRepoPath } from "../../../../../packages/core/src/state/substrate-project-service.ts";
import type { BeadDependency, BeadIssue } from "../../../../../packages/core/src/types/beads.ts";
...
type WriteLogFn = (op: string, outcome: WriteOutcome, startedAt: number, metadata: Record<string, unknown>) => void;
...
const MAX_BD_OUTPUT_BYTES = 1024 * 1024;
const MAX_BD_QUEUE_DEPTH = 16;
const BD_COMMAND_TIMEOUT_MS = 30_000;
const MAX_WRITE_BODY_BYTES = 256 * 1024;
const bdQueues = new Map<string, { tail: Promise<void>; pending: number }>();
...
export function createBeadsWriteRouter(xtrmDb?: Database | null, options: BeadsWriteRouterOptions = {}): Hono {
  ...
  const writeError: WriteErrorFn = (...args) => respondWriteError(options.emit, ...args);
  const logWrite: WriteLogFn = (...args) => emitWriteLog(options.emit, ...args);

  router.post("/projects/:projectId/issues", async (c) => {
    const authError = assertWriteAllowed(c.req.url, c.req.header("host") ?? "", c.req.header("origin") ?? null, c.req.header("x-console-write-token") ?? c.req.header("x-gitboard-sources-admin-token") ?? null, xtrmDb, c.req.header(TRUSTED_PEER_ADDRESS_HEADER));
    ...
    const args = ["-C", repoPath, ...MUTATION_PREFIX, "create", "--title", body.title];
    ...
    return await runIssueMutation({ c, projectId, op: "create", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
  ...
  router.patch("/projects/:projectId/issues/:issueId", async (c) => {
    ...
    return await runIssueMutation({ c, projectId, issueId, op: "update", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
  ...
  router.post("/projects/:projectId/issues/:issueId/close", async (c) => {
    ...
    return await runIssueMutation({ c, projectId, issueId, op: "close", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
  ...
  router.post("/projects/:projectId/issues/:issueId/reopen", async (c) => {
    ...
    return await runIssueMutation({ c, projectId, issueId, op: "reopen", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
  ...
  router.post("/projects/:projectId/issues/:issueId/comments", async (c) => {
    ...
    return await runIssueMutation({ c, projectId, issueId, op: "comment", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
  ...
  router.post("/projects/:projectId/issues/:issueId/notes", async (c) => {
    ...
    return await runIssueMutation({ c, projectId, issueId, op: "note", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
  ...
  router.post("/projects/:projectId/issues/:issueId/dependencies", async (c) => {
    ...
    return await runIssueMutation({ c, projectId, issueId, op: "dep-add", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
  ...
  router.post("/projects/:projectId/issues/:issueId/priority", async (c) => {
    ...
    return await runIssueMutation({ c, projectId, issueId, op: "priority", repoPath, args, runBdCommand, readIssue, writeError, logWrite });
  });
...
async function defaultRunBdCommand(repoPath: string, command: string[], _op: string): Promise<BdRunResult> {
  const queue = bdQueues.get(repoPath) ?? { tail: Promise.resolve(), pending: 0 };
  if (queue.pending >= MAX_BD_QUEUE_DEPTH) throw new Error("bd command queue is full");
  queue.pending += 1;
  let release!: () => void;
  const previous = queue.tail;
  queue.tail = new Promise<void>((resolve) => { release = resolve; });
  bdQueues.set(repoPath, queue);
  await previous;
  try {
    return await spawnBdCommand(repoPath, command);
  } finally {
    queue.pending -= 1;
    release();
    if (queue.pending === 0) bdQueues.delete(repoPath);
  }
}

function spawnBdCommand(repoPath: string, command: string[]): Promise<BdRunResult> {
  return new Promise((resolve, reject) => {
    ...
    const append = (target: Buffer[], chunk: Buffer | string) => {
      ...
      if (outputBytes > MAX_BD_OUTPUT_BYTES) {
        child.kill("SIGKILL");
        finish(() => reject(new Error("bd command output exceeded limit")));
        return;
      }
      target.push(value);
    };
    ...
    child.on("close", (exitCode) => {
      finish(() => resolve({ stdout: Buffer.concat(stdout).toString("utf8"), stderr: Buffer.concat(stderr).toString("utf8"), exitCode: exitCode ?? 1 }));
    });
    timeout = setTimeout(() => {
      child.kill("SIGKILL");
      finish(() => reject(new Error("bd command timed out")));
    }, BD_COMMAND_TIMEOUT_MS);
  });
}

function assertWriteAllowed(url: string, host: string, origin: string | null, requestToken: string | null, db?: Database | null, peerAddress?: string | null): string | null {
  if (!db) return "forbidden";
  return isAllowedConsoleWriteRequest(url, host, origin, requestToken, process.env, peerAddress) ? null : "forbidden";
}

async function safeJson<T>(request: Request): Promise<T | null | undefined> {
  const text = await readBoundedText(request, MAX_WRITE_BODY_BYTES);
...
function respondWriteError(
  emit: ((entry: LogEntry) => void) | undefined,
  c: JsonContext,
  op: string,
  outcome: WriteOutcome,
  message: string,
  ...
  emitWriteLog(emit, op, outcome, startedAt, { ...metadata, error: message });
  return c.json({ error: message }, status);
}

function emitWriteLog(emit: ((entry: LogEntry) => void) | undefined, op: string, outcome: WriteOutcome, startedAt: number, metadata: Record<string, unknown>): void {
