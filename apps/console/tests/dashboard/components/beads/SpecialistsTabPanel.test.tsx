...
describe("SpecialistsTabPanel", () => {
  ...
  it("matches current project jobs by beads project aliases, not owner/repo", async () => {
    ...
    expect(logClientEvent).toHaveBeenCalledWith("drawer.specialists.chip.clicked", expect.objectContaining({ beadId: "forge-1", jobId: "job-a", target: "inspector" }));
    expect(logClientEvent).toHaveBeenCalledWith("chip.inspector.dispatched", expect.objectContaining({ source: "drawer_row", beadId: "forge-1", jobId: "job-a" }));
  });
});
