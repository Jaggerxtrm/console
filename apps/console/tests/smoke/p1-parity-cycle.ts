...
async function main(): Promise<void> {
  ...
  if (logs.length <= before) throw new Error("expected parity log emission");

  const log = `${new Date().toISOString()} cycles=5 parity_ok_count=${harness.getParityOkCount()} diff_count=${summary.diff_count} log_events=${logs.filter((entry) => entry.event === "parity.observability").length}`;
