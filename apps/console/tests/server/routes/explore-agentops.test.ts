...
let db: Database;
let logger: ReturnType<typeof createHostLogger>;
...
describe("GET /api/console/explore/agentops", () => {
  it("returns filtered AgentOps summary, facets, breakdowns, and job rows", async () => {
    const app = new Hono().route("/api/console/explore", createExploreAgentopsRouter(db, { now: Date.UTC(2026, 5, 29), emit: logger.emit }));
  ...
  it("logs bounded filter metadata without raw SQL", async () => {
    ...
    const line = logger.getRing().find((entry) => entry.component === "explore" && entry.event === "agentops_request");
