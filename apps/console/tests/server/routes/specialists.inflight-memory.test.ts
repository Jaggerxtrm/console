...
import { createSpecialistsRouter, MAX_IN_FLIGHT_REFRESHES, MAX_REPO_SLUG_FILTERS, MAX_REPO_SLUG_FILTER_BYTES } from "../../../src/server/routes/specialists.ts";
import type { SpecialistJob } from "../../../../../packages/core/src/observability/types.ts";
...
describe("specialists in-flight memory bound", () => {
  function makeApp(reads: { inFlight: () => void; history: (limit: number) => void }, emit?: ReturnType<typeof createHostLogger>["emit"]): Hono {
    const app = new Hono();
  ...
  it("emits bounded response telemetry with cacheable specialist fields", async () => {
    ...
    const app = makeApp({ inFlight: () => {}, history: () => {} }, logger.emit);

    const response = await app.fetch(new Request("http://localhost/api/specialists/jobs/in-flight"));
    const entry = logger.getRing().find((item) => item.event === "specialists.in_flight.response");
