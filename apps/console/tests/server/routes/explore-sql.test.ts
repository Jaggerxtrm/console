...
describe("explore SQL proxy", () => {
  ...
  it("converts upstream 5xx into a bounded 502 envelope and logs hashed path", async () => {
    ...
    const app = new Hono().route("/explore/sql", createExploreSqlRouter({ datasetteUrl: "http://datasette.test", fetchImpl: fetchImpl as unknown as typeof fetch, emit: logger.emit }));
    ...
    const line = logger.getRing().find((entry) => entry.component === "explore" && entry.event === "proxy_request");
