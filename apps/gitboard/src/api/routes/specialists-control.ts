...
export function createSpecialistsControlRouter(xtrmDb: Database | null, deps: RouteDeps = {}): Hono {
  ...
  router.post("/jobs/:job_id/stop", async (c) => {
    ...
    await handleAction({ action: "stop", job: currentJob, env, runCommand, writeControlMessage });
  ...
  router.post("/jobs/:job_id/steer", async (c) => {
    ...
    const currentJob = readJob(jobId);
    if (!currentJob) return c.json({ error: "job not found" }, 404);
    await handleAction({ action: "steer", job: currentJob, env, runCommand, writeControlMessage, text: message });
    return c.json({ job: await readUpdatedJob(readJob, jobId, currentJob) });
  });
  ...
  router.post("/jobs/:job_id/resume", async (c) => {
    ...
    await handleAction({ action: "resume", job: currentJob, env, runCommand, writeControlMessage, text: task });
    return c.json({ job: await readUpdatedJob(readJob, jobId, currentJob) });
  });
...
async function handleAction(args: { action: ControlAction; job: SpecialistJob; env: NodeJS.ProcessEnv; runCommand: CommandRunner; writeControlMessage: ControlMessageWriter; text?: string }): Promise<void> {
  const startedAt = performance.now();
  const command = args.env.GITBOARD_SPECIALISTS_BIN || "sp";
  const repo = listRepos().find((entry) => entry.repoSlug === args.job.repoSlug);
  ...
  try {
    ...
    emit(makeLogEntry("api", "specialists.control", "info", undefined, {
      ...baseMetadata,
      outcome: "success",
      duration_ms: Math.round(performance.now() - startedAt),
    }));
  } catch {
    emit(makeLogEntry("api", "specialists.control", "warn", undefined, {
      ...baseMetadata,
      outcome: "error",
      duration_ms: Math.round(performance.now() - startedAt),
