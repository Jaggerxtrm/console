...
export function createExploreAgentopsRouter(db: Database | null | undefined, options: ExploreAgentopsOptions = {}): Hono {
  ...
  router.get("/agentops", (c) => {
    ...
    if (!db) {
      ...
      logAgentops("degraded", startedAt, filters, 0);
      return c.json(empty);
    }
    ...
    try {
      ...
      logAgentops("ok", startedAt, filters, rows.length);
      return c.json(response);
    } catch (error) {
      emit(makeLogEntry("explore", "agentops_request", "warn", "agentops query failed", {
        outcome: "error",
...
function logAgentops(outcome: "ok" | "degraded", startedAt: number, filters: ExploreAgentopsFilters, totalJobs: number): void {
  emit(makeLogEntry("explore", "agentops_request", outcome === "ok" ? "info" : "warn", undefined, {
    outcome,
    duration_ms: Math.max(0, Math.round(performance.now() - startedAt)),
    range: filters.range,
