  ...
  return {
    ...
    surfaces: [
      ...
      },
      {
        ...
        currentOwner: "apps/gitboard",
        targetOwner: "packages/core",
        currentPaths: ["apps/gitboard/src/api/ws/channels.ts", "apps/gitboard/src/api/ws/handler.ts", "apps/gitboard/src/core/logger.ts", "apps/gitboard/src/api/server.ts"],
        targetExport: "@xtrm/core/runtime",
        knownHighRiskSymbols: ["ChannelRegistry", "WsHandler", "emit"],
        preserves: ["websocket subscription protocol", "replay buffer behavior", "internal log ring", "post-commit sync hints", "request timing/error/slow logs"],
        deprecationGate: "Core owns realtime/log delivery contracts and app websocket handlers only adapt Bun upgrades.",
      ...
      },
      {
        ...
        preserves: ["gitboard.service compatibility", "HOST/PORT/GITBOARD_DATA_DIR envs", "/console static host", "/gitboard compatibility bundle", "manual production restart gate"],
        deprecationGate: "gitboard.service is a compatibility alias/wrapper over the core runtime target and no runtime surface remains app-owned.",
  ...
  return {
    ...
    children: [
      ...
      },
      {
        ...
        notes: "Keep /gitboard and gitboard.service until explicit compatibility-retirement gates pass.",
      },
      {
