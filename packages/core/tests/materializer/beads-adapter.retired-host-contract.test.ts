...
import { BeadsAdapter } from "../../src/materializer/beads-adapter.ts";
...
describe("BeadsAdapter", () => {
  it("delegates snapshot + diff and materializer advances cursor only on success", async () => {
    ...
    const adapter = new BeadsAdapter({ sourceKey: "beads:proj-1", projectId: "proj-1", xtrmDb, readSnapshot: () => readBeadsIssuesFromJsonl(beadsPath) });
