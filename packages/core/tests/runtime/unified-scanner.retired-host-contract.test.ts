...
import { normalizeLegacySourceStatus, UnifiedScanner } from "../../src/runtime/unified-scanner.ts";
...
describe("UnifiedScanner", () => {
  ...
  it("records non-missing probe failures as structured warnings", () => {
    ...
    const scannerWithPrivateMethods = new UnifiedScanner(db, { beadsSearchPath: tmpDir, observabilityRoots: [tmpDir], parityEnabled: false, emitLog: (entry) => entries.push(entry) }) as unknown as {
