...
import { createDatabase } from "../../src/github/database.ts";
import {
  transformEvent,
  transformCommits,
  GithubPoller,
...
import { getGithubToken } from "../../src/github/token.ts";
import { ensureRepo, getEvents, getCommits } from "../../src/github/index.ts";
import type { GithubActivityPublisher } from "../../src/github/ports.ts";
...
class CollectingPublisher implements GithubActivityPublisher {
  readonly events: Array<{ channel: string; event: string; data: unknown; version: string }> = [];
...
describe("GithubPoller", () => {
  ...
  it("ingestEvents stores events, commits, and publishes event append", async () => {
    const registry = new CollectingPublisher();
    const messages = registry.events;
